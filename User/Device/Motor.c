...
void DM_Motor_CAN_TxMessage_4310(FDCAN_TxFrame_TypeDef *TxFrame, DM_Motor_Info_Typedef *DM_Motor, uint8_t Mode, float Postion, float Velocity, float KP, float KD, float Torque){
	  ...
	  if(Mode == MIT_Mode) {
			 ...
			 //输出限幅：
			 Postion = DM_Motor_Limit_float(Postion, P_MAX_4310);
			 Velocity = DM_Motor_Limit_float(Velocity, V_MAX_4310);
			 Torque = DM_Motor_Limit_float(Torque, 3);
			
			 Postion_Tmp  =  float_to_uint(Postion,-P_MAX_4310,P_MAX_4310,16) ;
			 Velocity_Tmp =  float_to_uint(Velocity,-V_MAX_4310,V_MAX_4310,12);
			 Torque_Tmp = float_to_uint(Torque,-T_MAX_4310,T_MAX_4310,12);
...
// 4310电机控制
// param: Fdcan发送结构体，电机总结构体，电机模式，位置，速度，KP，KD，力矩
void DM_Motor_CAN_TxMessage_6215(FDCAN_TxFrame_TypeDef *TxFrame, DM_Motor_Info_Typedef *DM_Motor, uint8_t Mode, float Postion, float Velocity, float KP, float KD, float Torque){
	  ...
	  if(Mode == MIT_Mode) {
			 ...
			 //输出限幅：
			 Postion = DM_Motor_Limit_float(Postion, P_MAX_6215);
			 Velocity = DM_Motor_Limit_float(Velocity, V_MAX_6215);
			 Torque = DM_Motor_Limit_float(Torque, 1);
			
			 Postion_Tmp  =  float_to_uint(Postion,-P_MAX_6215,P_MAX_6215,16) ;
			 Velocity_Tmp =  float_to_uint(Velocity,-V_MAX_6215,V_MAX_6215,12);
			 Torque_Tmp = float_to_uint(Torque,-T_MAX_6215,T_MAX_6215,12);
			 KP_Tmp = float_to_uint(KP,0,500,12);
			 KD_Tmp = float_to_uint(KD,0,5,12);
...
void DM_Motor_Info_Update_4310(uint8_t *Data,DM_Motor_Info_Typedef *DM_Motor)
{
	  ...
	  DM_Motor->Data.State = Data[0]>>4;
		...
		DM_Motor->Data.Torque = uint_to_float(DM_Motor->Data.T_int,-T_MAX_4310,T_MAX_4310,12);
		DM_Motor->Data.Position = uint_to_float(DM_Motor->Data.P_int,-P_MAX_4310,P_MAX_4310,16);
    DM_Motor->Data.Velocity = uint_to_float(DM_Motor->Data.V_int,-V_MAX_4310,V_MAX_4310,12);
...
// 6215电机反馈数据解码
// param: 8字节返回数据，电机总结构体地址
void DM_Motor_Info_Update_6215(uint8_t *Data,DM_Motor_Info_Typedef *DM_Motor)
{
		DM_Motor->ID  = Data[0] & 0x0F;
	  DM_Motor->Data.State = Data[0]>>4;
		DM_Motor->Data.P_int = (uint16_t)(((Data[1]) <<8) | (Data[2]));
		DM_Motor->Data.V_int = (uint16_t)((Data[3]) <<4) | ((Data[4])>>4);
		DM_Motor->Data.T_int = (uint16_t)((Data[4]&0xF) <<8) | ((uint16_t)(Data[5]));
		DM_Motor->Data.Torque = uint_to_float(DM_Motor->Data.T_int,-T_MAX_6215,T_MAX_6215,12);
		DM_Motor->Data.Position = uint_to_float(DM_Motor->Data.P_int,-P_MAX_6215,P_MAX_6215,16);
    DM_Motor->Data.Velocity = uint_to_float(DM_Motor->Data.V_int,-V_MAX_6215,V_MAX_6215,12);
    DM_Motor->Data.Temperature_MOS   = (float)(Data[6]);
		DM_Motor->Data.Temperature_Rotor = (float)(Data[7]);
